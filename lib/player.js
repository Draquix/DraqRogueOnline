    ...
    this.attack = function(mob){
        ...
        if(rng>pack.chance){
            ...
            if(this.gear.Weapon.length>0){
                let wHit = this.gear.Weapon.dam;
                pack.msg += this.gear.Weapon.name + 'and hit for ';
                pack.damage += Math.floor(Math.random()*(wHit[1]-wHit[0])+wHit[1]+1);
                pack.msg += damage + 'hit points of damage!';
            } else {
