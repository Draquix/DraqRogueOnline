...
function Player(name,passphrase,id){
    ...
    this.attack = function(mob){
        let chance = .5 + this.agi/10 - mob.stats.def/10;
        if(this.combatStyle==='accurate'){
            chance = .5 + this.agi/5 - mob.stats.def/10;
        }
        let rng = Math.random();
        if(chance>rng){
            let damage = this.str;
            if(this.gear.Weapon.length>0){
                damage += Math.floor(Math.random()(this.gear.Weapon.dam[1]-this.gear.Weapon.dam[0])-this.gear.Weapon.dam[0]);
            }
            return {hit:true,dam:damage,chance:chance};
        } else {
            return {hit:false,chance:chance};
        }
    }
    this.defend = function(mob){
        let chance = .5 + mob.stats.agi/10 - this.def/10;
        if(this.combatStyle==='defensive'){
            chance = .5 + mob.stats.agi/10 - this.def/5;
        }
        let rng = Math.random();
        if(chance>rng){
            return {hit:false,chance:chance};
            ...
            return {hit:true,chance:chance,damage:damage};
        }
    }
    this.init = function(){
