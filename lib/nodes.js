...
function Forge(){
    this.name ="The Forge";
    this.metal1={name:"none",purity:0};
    this.metal2={name:"none",purity:0};
    this.inUse=false;
    this.ticker=0;
    this.recipes=["",
    [//forging 1
...
this.smelt = function(lvl,num){
    let item = this.recipes[lvl][num];
    console.log('smelting this item:',item)
    let allow1 = false;
    let allow2 = false;
    // console.log('smelting forge method: ', item);
    this.ticker=item.tick;
    if(item.metal1===this.metal1.name&&item.metal1!=item.metal2&&this.metal1.purity>=.5){
        this.metal1.purity -= .5;
        allow1 = true;
        console.log('first');
    } if(item.metal1===this.metal2.name&&item.metal1!=item.metal2&&this.metal2.purity>=.5){
        this.metal2.purity -= .5;
        allow1 = true;
        console.log('second');
    } if(item.metal2===this.metal1.name&&item.metal1!=item.metal2&&this.metal1.purity>=.5){
        this.metal1.purity -= .5;
        allow2 = true;
        console.log('third');
    } if(item.metal2===this.metal2.name&&item.metal1!=item.metal2&&this.metal2.purity>=.5){
        this.metal2.purity -= .5;
        allow2 = true;
        console.log('fourth');
    } else if(item.metal1===this.metal1.name&&item.metal1===item.metal2&&this.metal1.purity>=1){
        this.metal1.purity -= 1;
        allow1 = true;
        allow2 = true;
        console.log('fifth');
    } else if(item.metal1===this.metal2.name&&item.metal1===item.metal2&&this.metal2.purity>=1){
        this.metal2.purity -= 1;
        allow1 = true;
        allow2 = true;
        console.log('sixth');
    }
    // if(item.metal2===this.metal1.name&&item.metal2!=item.metal1&&this.metal2.purity>=.5){
    //     this.metal1.purity -= .5;
    //     allow1 = true;
    // } else if(item.metal2===this.metal2.name&&this.metal2.purity>=.5){
    //     this.metal2.purity -= .5;
    //     allow2 = true;
    // } 
    if(allow1===true&&allow2===true){
        ...
        console.log(allow1, allow2, this.metal1, this.metal2);
    }
};
this.empty= function(num){
    if(num===1){
        this.metal1.name="na";
