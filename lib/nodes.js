...
totaler: function(){
    ...
    for(i in this.contents){
        if(this.contents[i].metal===ore1.name){
            this.contents[i].purity += ore1.purity;
        } else {
...
addCheck: function(added){
    ...
    for(i in this.contents){
        if(this.contents[i].metal!=ore1){
            ore1=this.contents[i].metal;
        } else if (this.contents[i].metal!=ore2){
            ore2=this.contents[i].metal;
        }
    }
    console.log('loaded with ores: ',ore1, ore2);
    if(added.metal===ore1||added.metal===ore2){
...
smelt: function(cont,lvl,num){
    let item = this.recipes[lvl][num];
    // console.log('smelting forge method: ', item);
    var one=false,two=false;
    for(i in cont){
        if(cont[i]['metal']===item.metal1&&one===false){
            one=true;
            cont.splice(i,1);
        }
    }
    for(i in cont){
        if(cont[i]['metal']===item.metal2&&two===false){
            two=true;
            cont.splice(i,1);
        }
    }
