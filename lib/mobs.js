...
function Mob(name,ascii,color,hp,xp,str,agi,def,lH,HH,lg,Hg,attN){
    ...
    this.attack = function(player) {
        ...
        let rng = Math.random();
        // console.log('mob attack : ',chance,rng);
        if (rng>chance){
            let damage = Math.floor(Math.random()*(this.stats.HH-this.stats.lH)+this.stats.lH);
...
const mobsList = [
    ['Giant Rat','r','grey',6,2,1,1,1,1,2,2,2,'bite'],
    ['Kobold','k','yellow',8,3,2,1,2,3,2,3,4,'punch'],
    ['Slime','s','blue',5,3,1,2,2,1,2,2,3,'ooze spit']
]
